    ...
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-body)', fontFamily: "'Outfit', sans-serif" }}>
      <header style={{ marginBottom: 40 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ position: 'relative' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, phone..."
                style={{ padding: '12px 12px 12px 44px', borderRadius: 14, border: '1px solid var(--border-main)', width: 260, fontWeight: 600 }} />
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
          {[
            { label: 'Total Customers', value: stats.total, icon: <Users size={18} />, color: 'var(--theme-primary)', bg: '#eff6ff' },
          ...
          {filtered.map((c, index) => {
            ...
            return (
              <motion.div layout key={c.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}
                style={{ background: 'var(--bg-surface)', borderRadius: 28, padding: 28, border: '1px solid var(--border-main)', boxShadow: '0 4px 20px rgba(0,0,0,0.02)', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'start', gap: 16, marginBottom: 20 }}>
                  <div style={{ width: 56, height: 56, borderRadius: 18, background: 'var(--bg-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: tier.color, fontWeight: 900, fontSize: 20, border: `2px solid ${tier.bg}`, flexShrink: 0 }}>
                    {c.name[0]}
                  </div>
                ...
                {/* Loyalty Points Bar */}
                <div style={{ background: 'var(--bg-body)', borderRadius: 14, padding: '12px 16px', marginBottom: 16 }}>
                ...
                <div style={{ paddingTop: 16, borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <button onClick={() => handleOptions(c)} style={{ border: 'none', background: 'var(--bg-body)', borderRadius: 8, padding: '6px 12px', fontWeight: 700, fontSize: 12, cursor: 'pointer', color: 'var(--text-muted)' }}>
                  ...
                  <button onClick={async () => { setSelectedCustomer(c); await fetchHistory(c.id); setShowHistoryModal(true); }}
                    style={{ color: 'var(--theme-primary)', border: 'none', background: 'transparent', fontWeight: 800, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                {[
                  ...
                  <div key={field}>
                    ...
                    <input value={newCustomer[field]} onChange={e => setNewCustomer({...newCustomer, [field]: e.target.value})} placeholder={placeholder}
                      style={{ width: '100%', padding: '14px', borderRadius: 12, border: '1px solid var(--border-main)', fontWeight: 600 }} />
                  </div>
                ))}
                <button onClick={handleAddCustomer} style={{ marginTop: 12, padding: 18, borderRadius: 16, background: 'var(--theme-primary)', color: 'white', border: 'none', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center' }}>
      ...
      <AnimatePresence>
        {showPaymentModal && selectedCustomer && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 40 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 40 }} style={{ background: 'var(--bg-surface)', borderRadius: 32, width: '100%', maxWidth: 420, position: 'relative', padding: 40, boxShadow: '0 40px 100px rgba(0,0,0,0.2)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--text-muted)', marginBottom: 8 }}>PAYMENT AMOUNT ($)</label>
                  <input type="number" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} placeholder="0.00"
                    style={{ width: '100%', padding: '14px', borderRadius: 12, border: '1px solid var(--border-main)', fontWeight: 700, fontSize: 18 }} />
                </div>
                <div>
      ...
      <AnimatePresence>
        {showHistoryModal && selectedCustomer && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 40 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 40 }} style={{ background: 'var(--bg-surface)', borderRadius: 32, width: '90%', maxWidth: 560, position: 'relative', padding: 40, boxShadow: '0 40px 100px rgba(0,0,0,0.2)', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
                <div>
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {historyLoading ? (
                  ...
                  <div>
                    ...
                    <div style={{ background: 'var(--bg-body)', padding: 20, borderRadius: 20, marginBottom: 24, border: '1px solid var(--border-main)' }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--text-main)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
                        <TrendingUp size={16} color="var(--theme-primary)" /> Purchase History Trend ($)
                      </div>
                        ...
                        {history.slice(0, 15).reverse().map((h, i) => {
                          ...
                          return (
                            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, minWidth: 28, gap: 6 }}>
                              <span style={{ fontSize: 10, fontWeight: 800, color: 'var(--theme-primary)' }}>${val.toFixed(0)}</span>
                              <div 
                                title={`$${val.toFixed(2)} on ${new Date(h.createdAt).toLocaleDateString()}`}
                                style={{ 
                                  width: '100%', 
                                  height: `${heightPct}%`, 
                                  background: 'linear-gradient(180deg, var(--theme-primary), #60a5fa)', 
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {history.map((item, idx) => (
                        <div key={idx} style={{ padding: '16px 20px', borderRadius: 16, background: 'var(--bg-body)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div>
                          ...
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontWeight: 900, fontSize: 16, color: 'var(--text-main)' }}>${parseFloat(item.grandTotal).toFixed(2)}</div>

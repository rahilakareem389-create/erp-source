...
const MetricCard = ({ title, value, sub, trend, color = 'var(--theme-primary)', delay = 0 }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ delay }}
    style={{
      background: 'var(--bg-surface)', padding: 28, borderRadius: 28,
    ...
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-body)', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <header style={{ marginBottom: 32, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          {/* Date range filter */}
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: 'var(--bg-surface)', padding: '8px 16px', borderRadius: 14, border: '1px solid var(--border-main)' }}>
          ...
          <button onClick={fetchAll}
            style={{ padding: '10px 20px', borderRadius: 14, background: 'var(--bg-surface)', border: '1px solid var(--border-main)', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, marginBottom: 28 }}>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              style={{ background: 'var(--bg-surface)', borderRadius: 28, padding: 36, border: '1px solid var(--border-main)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <input type="date" value={pnlFrom} onChange={e => setPnlFrom(e.target.value)}
                    style={{ padding: '6px 10px', borderRadius: 10, border: '1.5px solid var(--border-main)', fontFamily: "'Outfit', sans-serif", fontWeight: 700, outline: 'none' }} />
              ...
              {pnl && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                  ...
                  <div style={{ marginTop: 16, height: 8, background: 'var(--bg-hover)', borderRadius: 4, overflow: 'hidden' }}>
                  ...
                  return (
                    <div key={s.userId}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                        ...
                        <div style={{ fontWeight: 900, color: 'var(--theme-primary)', fontSize: 15 }}>{fmtCurrency(s.revenueGenerated)}</div>
                      </div>
                      <div style={{ height: 6, background: 'var(--bg-hover)', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${pct}%`, background: i === 0 ? '#f59e0b' : 'var(--theme-primary)', borderRadius: 3, transition: 'width 0.5s ease' }} />
                      </div>
                    </div>
                  );
                })}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            style={{ background: 'var(--bg-surface)', borderRadius: 28, padding: 36, border: '1px solid var(--border-main)' }}>
            <h2 style={{ fontSize: 20, fontWeight: 900, color: 'var(--text-main)', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 10 }}>
              <Package size={22} color="var(--theme-primary)" /> Top Selling Products
            </h2>

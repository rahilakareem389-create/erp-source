    ...
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-body)', fontFamily: "'Outfit', sans-serif" }}>
      <header style={{ marginBottom: 40, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <div style={{ display: 'flex', background: 'var(--bg-surface)', padding: '6px', borderRadius: 16, border: '1px solid var(--border-main)' }}>
          ...
          <button
            ...
            style={{ padding: '12px 24px', borderRadius: 16, background: auditData.length === 0 ? '#e2e8f0' : 'var(--theme-primary)', color: auditData.length === 0 ? '#94a3b8' : 'white', border: 'none', fontWeight: 800, cursor: auditData.length === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 40 }}>
        {[
          { label: 'Total Estimated Payroll', value: `$${totalEarnings.toFixed(2)}`, color: 'var(--theme-primary)', icon: <DollarSign size={20} /> },
          ...
          <div key={i} style={{ background: 'var(--bg-surface)', padding: 24, borderRadius: 24, border: '1px solid var(--border-main)' }}>
                ...
                auditData.map((item, i) => {
                  ...
                  return (
                    <tr key={i} onClick={() => setSelectedRow(item)}
                      ...
                      <td style={{ padding: '16px', fontSize: 13, fontWeight: 700, color: 'var(--text-main)' }}>{dateStr}</td>
                      ...
                      <td style={{ padding: '16px', fontSize: 13, fontWeight: 600 }}>
                        {clockOutTime ? <span style={{ color: '#ef4444' }}>{clockOutTime}</span> : <span style={{ color: 'var(--theme-primary)' }}>Active</span>}
                      </td>
        ...
        {selectedRow && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }}
              style={{ background: 'var(--bg-surface)', borderRadius: 28, width: '100%', maxWidth: 480, position: 'relative', padding: 36, zIndex: 1 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  ...
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', borderRadius: 12, background: 'var(--bg-body)' }}>

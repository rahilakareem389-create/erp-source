  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div style={{ background: '#0a0f1e', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '60px', position: 'relative', overflow: 'hidden' }}>
        {/* ambient glow */}
        <div style={{ position: 'absolute', top: '-10%', left: '-10%', width: 500, height: 500, background: 'radial-gradient(circle, rgba(var(--theme-primary-rgb),0.18) 0%, transparent 70%)', filter: 'blur(80px)', pointerEvents: 'none' }} />
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={() => navigate('/')}>
            <div style={{ width: 44, height: 44, borderRadius: 14, background: 'linear-gradient(135deg, var(--theme-primary), #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Zap size={22} fill="white" color="white" />
            </div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 900, color: 'white', letterSpacing: '-0.02em' }}>{companyName}</div>
              <div style={{ fontSize: 10, color: 'var(--text-main)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>ERP v2.0</div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'inline-block', padding: '6px 14px', borderRadius: 100, background: 'rgba(var(--theme-primary-rgb),0.12)', color: 'var(--theme-primary)', fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 24, border: '1px solid rgba(var(--theme-primary-rgb),0.2)' }}>
            Secure Employee Portal
          </div>
          <h1 style={{ fontSize: 52, fontWeight: 900, lineHeight: 1.1, letterSpacing: '-0.04em', color: 'white', marginBottom: 20 }}>
            Your department.<br /><span style={{ background: 'linear-gradient(135deg, var(--theme-primary), #6366f1)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Your portal.</span>
          </h1>
            ...
            {portals.map((p, i) => (
              <motion.div key={p.role} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.1 }}
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-main)', fontWeight: 600 }}>{p.path}</div>
                </div>
      ...
      {/* Right Panel — Form */}
      <div style={{ background: 'var(--bg-body)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 80px', position: 'relative' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} style={{ width: '100%', maxWidth: 420 }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type="email" placeholder="your@email.com" required
                  style={{ width: '100%', padding: '14px 14px 14px 46px', borderRadius: 14, border: '1.5px solid var(--border-main)', outline: 'none', fontFamily: "'Outfit', sans-serif", fontSize: 15, fontWeight: 600, color: 'var(--text-main)', background: 'var(--bg-surface)', transition: 'border-color 0.2s', boxSizing: 'border-box' }}
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type={showPass ? 'text' : 'password'} placeholder="••••••••" required
                  style={{ width: '100%', padding: '14px 46px 14px 46px', borderRadius: 14, border: '1.5px solid var(--border-main)', outline: 'none', fontFamily: "'Outfit', sans-serif", fontSize: 15, fontWeight: 600, color: 'var(--text-main)', background: 'var(--bg-surface)', transition: 'border-color 0.2s', boxSizing: 'border-box' }}
                  onFocus={e => e.target.style.borderColor = 'var(--theme-primary)'}
                  onBlur={e => e.target.style.borderColor = '#e2e8f0'}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
              <button type="button" onClick={() => setShowForgotPass(true)} style={{ background: 'none', border: 'none', color: 'var(--theme-primary)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                Forgot Password?
              </button>
            </div>

            <motion.button type="submit" disabled={loading}
              whileHover={!loading ? { scale: 1.02, boxShadow: '0 16px 40px rgba(var(--theme-primary-rgb),0.3)' } : {}}
              whileTap={!loading ? { scale: 0.98 } : {}}
              style={{ padding: '16px', borderRadius: 14, background: loading ? '#93c5fd' : 'linear-gradient(135deg, var(--theme-primary), #6366f1)', color: 'white', fontWeight: 800, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', fontSize: 16, fontFamily: "'Outfit', sans-serif", marginTop: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
              {loading ? (
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }} style={{ background: 'var(--bg-surface)', borderRadius: 24, width: '100%', maxWidth: 400, position: 'relative', padding: 32 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: '#eff6ff', color: 'var(--theme-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Lock size={20} /></div>
              ...
              <button onClick={() => setShowForgotPass(false)} style={{ width: '100%', padding: '14px', borderRadius: 12, background: 'var(--theme-primary)', color: 'white', fontWeight: 800, border: 'none', cursor: 'pointer' }}>
                Understood
              </button>
            </motion.div>
          </div>

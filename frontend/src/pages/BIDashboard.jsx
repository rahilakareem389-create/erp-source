  ...
  return (
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-body)', direction: isRTL ? 'rtl' : 'ltr' }}>
      <header style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 32, fontWeight: 900, color: 'var(--text-main)' }}>{t('biDashboard')}</h1>
        <p style={{ color: 'var(--text-muted)', fontWeight: 600 }}>Executive insights on project profitability and labor costs.</p>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, marginBottom: 32 }}>
        <div style={{ background: 'var(--bg-surface)', borderRadius: 24, padding: 32, border: '1px solid var(--border-main)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <div style={{ width: 48, height: 48, borderRadius: 16, background: '#eff6ff', color: 'var(--theme-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Briefcase size={24}/></div>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)' }}>Total Project Value</div>
          </div>
          <div style={{ fontSize: 32, fontWeight: 900, color: 'var(--text-main)' }}>SAR {(totalBudget * 1.5).toLocaleString()}</div>
        </div>
        ...
        <div style={{ background: 'var(--bg-surface)', borderRadius: 24, padding: 32, border: '1px solid var(--border-main)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)' }}>Monthly Labor Cost</div>
          </div>
          <div style={{ fontSize: 32, fontWeight: 900, color: 'var(--text-main)' }}>SAR {totalLaborCost.toLocaleString()}</div>
        </div>
        ...
        <div style={{ background: 'var(--bg-surface)', borderRadius: 24, padding: 32, border: '1px solid var(--border-main)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)' }}>Est. Margin</div>
          </div>
          <div style={{ fontSize: 32, fontWeight: 900, color: 'var(--text-main)' }}>25.4%</div>
        </div>
      </div>
      ...
      <div style={{ background: 'var(--bg-surface)', borderRadius: 24, padding: 40, border: '1px solid var(--border-main)' }}>
        <h2 style={{ fontSize: 20, fontWeight: 900, color: 'var(--text-main)', marginBottom: 32 }}>Project Profitability Analysis</h2>
            ...
            return (
              <div key={p.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                  <span style={{ fontWeight: 800, color: 'var(--text-main)', fontSize: 15 }}>{p.name}</span>
                ...
                <div style={{ width: '100%', height: 24, display: 'flex', borderRadius: 12, overflow: 'hidden' }}>
                  <div style={{ width: `${(laborCost / revenue) * 100}%`, background: 'var(--theme-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 10, fontWeight: 800 }} title="Labor Cost">L</div>
                ...
                <div style={{ display: 'flex', gap: 16, marginTop: 12, fontSize: 12, fontWeight: 700, color: 'var(--text-muted)' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><div style={{ width: 8, height: 8, borderRadius: 4, background: 'var(--theme-primary)' }}/> Labor: SAR {laborCost.toLocaleString()}</span>

...
const inputStyle = {
  ...
  border: '1.5px solid var(--border-main)', fontSize: 14, fontWeight: 600,
      ...
      <header style={{ marginBottom: 40 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button onClick={fetchAll} style={{ padding: '12px', borderRadius: 14, background: 'var(--bg-surface)', border: '1px solid var(--border-main)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 24, marginBottom: 48 }}>
          {supplierStats.map(s => (
            <motion.div key={s.id} whileHover={{ y: -4, boxShadow: '0 12px 40px rgba(0,0,0,0.10)' }} transition={{ type: 'spring', stiffness: 300 }}
              style={{ background: 'var(--bg-surface)', padding: 28, borderRadius: 28, border: '1px solid var(--border-main)', display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <span style={{ display: 'inline-block', padding: '4px 10px', borderRadius: 20, background: 'var(--bg-hover)', color: 'var(--text-muted)', fontSize: 10, fontWeight: 800, marginBottom: 6 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                {[
                  ...
                  <div key={m.label} style={{ textAlign: 'center', padding: '10px 6px', borderRadius: 12, background: 'var(--bg-body)' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => openEdit(s)}
                  style={{ padding: '12px', borderRadius: 14, background: 'var(--bg-hover)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {['all','pending','received'].map(f => (
              <span key={f} style={{ padding: '6px 14px', borderRadius: 20, background: 'var(--bg-hover)', color: 'var(--text-muted)', fontWeight: 700, fontSize: 12, cursor: 'default', textTransform: 'capitalize' }}>{f}</span>
            ))}
          </div>
        </div>
        <div style={{ background: 'var(--bg-surface)', borderRadius: 28, overflow: 'hidden', border: '1px solid var(--border-main)' }}>
                  ...
                  <React.Fragment key={o.id}>
                    <tr style={{ borderBottom: '1px solid #f8fafc', transition: 'background 0.15s' }}
                      ...
                      <td style={{ padding: '16px 12px 16px 18px', width: 36 }}>
                        {items.length > 0 && (
                          <button onClick={() => setExpandedOrder(isExpanded ? null : o.id)}
                            style={{ width: 28, height: 28, borderRadius: 8, border: '1px solid var(--border-main)', background: isExpanded ? '#f1f5f9' : 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {isExpanded ? <ChevronUp size={14} color="#64748b" /> : <ChevronDown size={14} color="#64748b" />}
                          </button>
                        )}
                      </td>

                      <td style={{ padding: '16px 18px', fontFamily: 'monospace', fontSize: 12, fontWeight: 800, color: 'var(--text-main)' }}>
                      ...
                      <td style={{ padding: '16px 18px' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 10, background: 'var(--bg-hover)', fontSize: 12, fontWeight: 700, color: 'var(--text-main)' }}>
                            ...
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
                              <div style={{ padding: '16px 56px 20px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                  {items.map((item, idx) => (
                                    <div key={item.id || idx} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 14px', background: 'var(--bg-surface)', borderRadius: 10, border: '1px solid var(--border-main)' }}>
                                      ...
                                      <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
                                        Unit: <strong style={{ color: 'var(--text-main)' }}>${parseFloat(item.unitCost || 0).toFixed(2)}</strong>
                                      </div>
        ...
        {showOrderModal && selectedSupplier && (
          <ModalWrapper onClose={() => setShowOrderModal(false)}>
            <ModalCard maxWidth={680}>
              <ModalHeader
                title={<span>New PO — <span style={{ color: 'var(--theme-primary)' }}>{selectedSupplier.name}</span></span>}
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    ...
                    <button type="button" onClick={addLine}
                      style={{ padding: '6px 14px', borderRadius: 10, background: 'var(--bg-hover)', border: 'none', color: 'var(--text-main)', fontWeight: 800, cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {poLines.map((line, idx) => (
                      <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        {/* Subtotal (read-only) */}
                        <div style={{ padding: '11px 12px', borderRadius: 12, background: 'var(--bg-body)', fontSize: 13, fontWeight: 800, color: 'var(--text-main)', textAlign: 'right', border: '1.5px solid var(--border-main)' }}>
...
const ModalCard = ({ children, maxWidth = 520 }) => (
  <motion.div initial={{ scale: 0.92, y: 30, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.92, y: 20, opacity: 0 }}
    style={{ background: 'var(--bg-surface)', borderRadius: 32, width: '100%', maxWidth, position: 'relative', padding: 40, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,0.18)' }}>
    {children}
  </motion.div>
);
...
const ModalHeader = ({ title, onClose }) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
    ...
    <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: 10, border: '1px solid var(--border-main)', background: 'var(--bg-surface)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <>
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 12, border: '1.5px solid var(--border-main)', background: '#fafafa' }}>

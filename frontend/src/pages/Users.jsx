  ...
  return (
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-body)', fontFamily: "'Outfit', sans-serif" }}>
      <header style={{ marginBottom: 32 }}>
        ...
        {/* Tab Bar */}
        <div style={{ display: 'flex', gap: 4, background: 'var(--bg-surface)', borderRadius: 16, padding: 6, width: 'fit-content', border: '1px solid var(--border-main)' }}>
          {TABS.map(tab => (
          ...
          {users.filter(u => u.name?.toLowerCase().includes(search.toLowerCase())).map(user => {
            ...
            return (
              <motion.div whileHover={{ y: -5 }} key={user.id}
                style={{ background: 'var(--bg-surface)', borderRadius: 28, padding: 32, border: '1px solid var(--border-main)', boxShadow: '0 4px 12px rgba(0,0,0,0.02)', opacity: isActive ? 1 : 0.65 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 24 }}>
                  <div style={{ position: 'relative' }}>
                    <div style={{ width: 64, height: 64, borderRadius: 20, background: isActive ? '#f1f5f9' : '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--theme-primary)', fontSize: 24, fontWeight: 900 }}>
                      {user.name.charAt(0)}
                    </div>
      ...
      {activeTab === 'System Settings' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          {[
            ...
            <div key={section} style={{ background: 'var(--bg-surface)', borderRadius: 24, padding: 32, border: '1px solid var(--border-main)' }}>
      ...
      <AnimatePresence>
        {showModal && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }} style={{ background: 'var(--bg-surface)', borderRadius: 32, width: '100%', maxWidth: 450, position: 'relative', padding: 40 }}>
              ...
              <form onSubmit={handleCreateUser} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <input required placeholder="Full Name" value={form.name} onChange={e => setForm({...form, name: e.target.value})} style={{ width: '100%', padding: '14px 16px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }} />
                <input required type="email" placeholder="Email Address" value={form.email} onChange={e => setForm({...form, email: e.target.value})} style={{ width: '100%', padding: '14px 16px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }} />
                <input required placeholder="Phone Number" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} style={{ width: '100%', padding: '14px 16px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }} />
                <select value={form.role} onChange={e => setForm({...form, role: e.target.value})} style={{ width: '100%', padding: '14px 16px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600, background: 'var(--bg-surface)' }}>
                ...
                <input type="password" placeholder="Temporary Password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} style={{ width: '100%', padding: '14px 16px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }} />

...
const SalaryAdvancePage = () => {
  ...
  const inputStyle = {
    ...
    border: '1.5px solid var(--border-main)', outline: 'none',
    fontFamily: "'Outfit', sans-serif", fontWeight: 600,
    fontSize: 15, color: 'var(--text-main)', background: 'var(--bg-body)',
    ...
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-body)', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <header style={{ marginBottom: 36, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <button
          ...
          style={{
            ...
            background: hasPending ? '#e2e8f0' : 'linear-gradient(135deg, var(--theme-primary), #6366f1)',
            color: hasPending ? '#94a3b8' : 'white', border: 'none',
            fontWeight: 800, cursor: hasPending ? 'not-allowed' : 'pointer',
            display: 'flex', alignItems: 'center', gap: 10, fontSize: 14,
            boxShadow: hasPending ? 'none' : '0 4px 20px rgba(var(--theme-primary-rgb),0.35)',
          }}
        >
          <CreditCard size={18} />
      ...
      {!isHR && (
        <div style={{ marginBottom: 28, padding: '16px 24px', borderRadius: 16, background: '#eff6ff',
          ...
          <Info size={20} color="var(--theme-primary)" style={{ flexShrink: 0, marginTop: 1 }} />
      ...
      {isHR && (
        <div style={{ marginBottom: 28, display: 'flex', gap: 4, background: 'var(--bg-surface)', padding: 4,
          borderRadius: 16, border: '1px solid var(--border-main)', alignSelf: 'flex-start', width: 'fit-content' }}>
          {[
      ...
      {(!isHR || activeTab === 'my') && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginBottom: 28 }}>
            {[
              { label: 'Total Requested', value: fmtMoney(myAdvances.reduce((s, a) => s + parseFloat(a.amount || 0), 0)), color: 'var(--theme-primary)', icon: <DollarSign size={22} /> },
              ...
              <motion.div key={card.label} whileHover={{ y: -2 }}
                style={{ background: 'var(--bg-surface)', padding: '24px 28px', borderRadius: 24, border: '1px solid var(--border-main)',
          ...
          {/* My advance history */}
          <div style={{ background: 'var(--bg-surface)', borderRadius: 28, border: '1px solid var(--border-main)', padding: 32 }}>
      ...
      {isHR && activeTab === 'hr' && (
        <div style={{ background: 'var(--bg-surface)', borderRadius: 28, border: '1px solid var(--border-main)', padding: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
            <div>
              <h2 style={{ fontSize: 20, fontWeight: 900, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: 10, margin: 0 }}>
                <ShieldCheck size={22} color="var(--theme-primary)" /> Advance Approval Queue
              </h2>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontWeight: 700, color: 'var(--text-main)' }}>{adv.deductionMonths} month{adv.deductionMonths > 1 ? 's' : ''}</div>
                          </div>
                          <div>
                            <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-muted)', marginBottom: 2 }}>SUBMITTED</div>
                            <div style={{ fontWeight: 700, color: 'var(--text-main)' }}>{fmtDate(adv.createdAt)}</div>
                          </div>
                          {adv.reason && (
                      ...
                      {adv.status === 'pending' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
                          {isSelf ? (
                            <div style={{ padding: '10px 16px', borderRadius: 12, background: 'var(--bg-hover)',
                              color: 'var(--text-muted)', textAlign: 'center', fontWeight: 700, fontSize: 12, whiteSpace: 'nowrap' }}>
                              ⛔ Cannot self-approve
                            </div>
                          ) : (
                            <>
                              ...
                              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                                ...
                                style={{ padding: '11px 20px', borderRadius: 12, background: 'var(--bg-surface)', color: '#ef4444',
        ...
        {showForm && (
          ...
          >
            <motion.div
              ...
              style={{ background: 'var(--bg-surface)', borderRadius: 32, padding: 44, width: '100%', maxWidth: 500,
                boxShadow: '0 32px 80px rgba(0,0,0,0.22)' }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 8 }}>
                <div style={{ width: 52, height: 52, borderRadius: 16, background: '#eff6ff',
                  display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <CreditCard size={24} color="var(--theme-primary)" />
                </div>
                <div>
              ...
              <div style={{ height: 1, background: 'var(--bg-hover)', margin: '24px 0' }} />
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                  <button type="button" onClick={() => { setShowForm(false); setFormError(''); }}
                    style={{ flex: 1, padding: '15px', borderRadius: 16, background: 'var(--bg-hover)', border: 'none', fontWeight: 800, cursor: 'pointer', fontSize: 15 }}>
                    Cancel
                  </button>
                  <motion.button type="submit" disabled={submitting} whileHover={{ scale: submitting ? 1 : 1.02 }} whileTap={{ scale: 0.97 }}
                    style={{ flex: 2, padding: '15px', borderRadius: 16,
                      background: submitting ? '#94a3b8' : 'linear-gradient(135deg, var(--theme-primary), #6366f1)',
                      ...
                      boxShadow: submitting ? 'none' : '0 4px 16px rgba(var(--theme-primary-rgb),0.35)' }}>
                    <Send size={17} />

...
const Expenses = () => {
  ...
  const categories = [
    { name: 'Utilities', icon: <Zap size={18} />, color: 'var(--theme-primary)' },
  ...
  return (
    <div style={{ padding: 40, minHeight: '100vh', background: 'var(--bg-body)', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 40 }}>
        <div style={{ background: 'var(--bg-surface)', padding: 32, borderRadius: 32, border: '1px solid var(--border-main)', gridColumn: 'span 2' }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 8 }}>TOTAL ACCUMULATED BURN</div>
          <div style={{ fontSize: 48, fontWeight: 900, color: 'var(--text-main)' }}>${totalExpense.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        ...
        <div style={{ background: 'var(--theme-primary)', padding: 32, borderRadius: 32, color: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ background: 'var(--bg-surface)', borderRadius: 32, border: '1px solid var(--border-main)', padding: 32 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {filteredExpenses.map(expense => (
              <div key={expense.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 24px', borderRadius: 20, background: 'var(--bg-body)', border: '1px solid transparent' }}>
                <div style={{ width: 44, height: 44, borderRadius: 14, background: 'var(--bg-surface)', border: '1px solid var(--border-main)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--theme-primary)' }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          ...
          <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} style={{ background: 'var(--bg-surface)', borderRadius: 32, width: '100%', maxWidth: 450, position: 'relative', padding: 40 }}>
            ...
            <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <select value={form.category} onChange={e => setForm({...form, category: e.target.value})} style={{ width: '100%', padding: '14px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }}>
                  {categories.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
                </select>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 800, color: 'var(--text-muted)', marginBottom: 8 }}>AMOUNT ($)</label>
                <input required type="number" step="0.01" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} placeholder="0.00" style={{ width: '100%', padding: '14px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 800, color: 'var(--text-muted)', marginBottom: 8 }}>DESCRIPTION</label>
                <input required value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="e.g. Monthly Electricity Bill" style={{ width: '100%', padding: '14px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 800, color: 'var(--text-muted)', marginBottom: 8 }}>DATE</label>
                <input required type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} style={{ width: '100%', padding: '14px', borderRadius: 16, border: '1px solid var(--border-main)', fontSize: 14, fontWeight: 600 }} />
              </div>
